<script>
	...
	import Text from "$components/MiddleSchool_VIDEO/MiddleSchool.Text.svelte";
	import EyeContainer from "$components/MiddleSchool_VIDEO/MiddleSchool.EyeContainer.svelte";
...
<div class="outsideContainer reducedMotion{prefersReducedMotion}">
	<section id="scrolly">
	...
	<!-- <Scrolly bind:value top={0}>
		...
		<div class="step {step_obj.addclass ? step_obj.addclass : 'smallText'}" class:active>
			<Text copy={step_obj.text} type={step_obj.type} note="main"/>
		</div>
		{/if}
		{/each}
	</Scrolly> -->
	<div class="debugstep">{value}<br>{currentTime}<br>{nextTime}</div>
	</section>
	{#if copy.story[value].quote_id}
		{:else if copy.story[value].text != ""}
		<div class="step smallText">
			<Text copy={copy.story[value].text} type={copy.story[value].type} note="main"/>
		</div>
		{/if}
</div>
